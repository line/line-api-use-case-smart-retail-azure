...
const VueUtils = (app, store) => {
    ...
    const _functions = {
        ...
        getIOSVersion() {
            ...
            return parseFloat(version[0].replace('os ', '').replace(/_/g, '.'));
        },
         ...
         *　Androidバージョン取得
         *
         * @return {number} Androidバージョン
         */
        getAndroidVersion() {
            const userAgent = window.navigator.userAgent.toLowerCase();
            let version = userAgent.match(/(^\d+\.\d+|\d)|^\d+/).shift();
        ...
        showHttpError(error, errMsg) {
            if (error.response && error.response.status >= 400) {
                ...
                app.$popup.show("error", "Server Error", `status=${response.status} ${response.statusText} ${response.data}`);
                return true;
            } else {
                app.$popup.show("error", error.message, _i18n.utils.error01); // 通信エラーが発生しました。
            }
            return false;
        },
